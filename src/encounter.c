...
#define EXP_COST(monster_id) CHALLENGE_TABLE[monster_data_by_id(monster_id)->cr].experience
...
add_monster_to_encounter(size_t monster_id, Encounter* encounter)
{
    ...
    if (encounter->number_of_enemies < ENCOUNTER_SIZE_MULT_MAX && encounter->number_of_enemies > 1) {
        size_t current_index = encounter->number_of_enemies - 1;
        float inverse_factor = ENCOUNTER_SIZE_MULT[current_index-1] / ENCOUNTER_SIZE_MULT[current_index];
...
#define ASSERT_VALID_MONSTER(monster_id) assert(monster_id >= 0 && monster_id < number_monsters)

static size_t
select_seed_monster(enum Challenge lower_bound, enum Challenge upper_bound, RNG* rng)
{
    if (running_totals_by_cr[0] == MAX_MONSTERS)
        precompute_running_totals_by_cr_array();
...
generate_encounter(EncounterInputs* inputs, Encounter* encounter, RNG* rng)
{
    ...
    unsigned attempts = 0;
    do {
        seed_monster_id = select_seed_monster(choice.lower_bound, choice.upper_bound, rng);
